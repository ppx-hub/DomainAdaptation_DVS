...
def main():
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(output_base, 'train_TCKA_test_vis', exp_name)
    ...
    if args.eval_checkpoint == '':
        pass
        writer = SummaryWriter('runs/experiment_origin')
    else:
        model.load_state_dict(torch.load(args.eval_checkpoint)['state_dict'])
        writer = SummaryWriter('runs/experiment_transfer')
        print("load model!")
...
def train_epoch(
    ...
    for batch_idx, (inputs, label) in enumerate(target_loader):
        ...
        with amp_autocast():
            ...
            rgb_feature = rearrange(sum(domain_rbg_list), 'b c h w -> b (c h w)')
            dvs_feature = rearrange(sum(domain_dvs_list), 'b c h w -> b (c h w)')
            # log embeddings
            global writer
            writer.add_embedding(torch.cat([rgb_feature, dvs_feature], dim=0),
                                 metadata=torch.cat([label, label], dim=0),
                                 tag='rgb and dvs')
            writer.close()
