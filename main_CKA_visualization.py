...
parser.add_argument('--output', default='/home/hexiang/TransferLearning_For_DVS/Results_additional/', type=str, metavar='PATH',
...
def main():
    ...
    if args.local_rank == 0:
        ...
        exp_name = '-'.join([
            ...
            "refined_{}".format(not args.eval_checkpoint == "")
        ])
    ...
    if args.eval_checkpoint != '':
        ...
        old_dict = torch.load(args.eval_checkpoint)['state_dict']
        for key in old_dict:
...
if __name__ == '__main__':
    ...
    kde = stats.gaussian_kde(numpy_array)
    #
    # # 定义要在其上绘制PDF的值的范围
    # x = np.linspace(numpy_array.min(), numpy_array.max(), 1000)
    #
    # # 计算每个值的PDF
    # pdf = kde.evaluate(x)
    #
    # # 使用matplotlib的plot函数绘制PDF
    # plt.bar(x, pdf, width=(numpy_array.max() - numpy_array.min()) / 1000.)
    # plt.xlabel('Value')
    # plt.ylabel('Density')
    # plt.title('Tensor Data Probability Density Function')
    # plt.grid(True)
    # plt.show()
